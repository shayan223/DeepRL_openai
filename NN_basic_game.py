...
'''Code from the following tutorial https://gist.github.com/Pocuston/13f1a7786648e1e2ff95bfad02a51521'''
...
class Network(nn.Module):
    def __init__(self):
        ...
        self.d2 = nn.Linear(HIDDEN_LAYER,HIDDEN_LAYER)
        self.l2 = nn.Linear(HIDDEN_LAYER, 2)
        self.leakyrelu = nn.LeakyReLU(0.1)

    def forward(self, x):
        x = self.leakyrelu(self.l1(x))
        x = self.leakyrelu(self.d2(x))
        x = self.leakyrelu(self.d2(x))
        x = self.l2(x)
        return x
