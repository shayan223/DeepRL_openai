...
def cartpole_control(test_number=1,adversarial=False,display=False,perturbation_strength=0.01,linear_p=True):
        ...
        for t in range(steps_per_epoch):
            ...
            if(adversarial == True):
                if(linear_p == True):
                    ...
                    change = create_noise(observation)*perturbation_strength
    ...
    if(adversarial == False):
        ...
        plt.plot(list(range(len(episode_scores))),episode_scores)
        #plt.plot(survival_lengths,list(range(len(survival_lengths))))
        plt.title('Testing Performance: Perturbation Strength='+str(perturbation_strength))
        plt.xlabel('Episode')
        plt.ylabel('Score')
        plt.savefig('./basic_nn/control_adversarial'+str(test_number)+'.jpg')
        plt.clf()
    '''
